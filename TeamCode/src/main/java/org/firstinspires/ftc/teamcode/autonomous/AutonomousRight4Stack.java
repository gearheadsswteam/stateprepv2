...
@Autonomous(name = "Right4Stack", group = "Right")
public class AutonomousRight4Stack extends AbstractAutonomous {
        ...
        while (opModeIsActive() && !isStopRequested() && (!traj5Done || time > retractTime)) {
            ...
            if (dropTrajDone && time - dropTrajTime > 0.5) {
                if (cycles == 0) {
                    ...
                    dropTrajDone = false;
                } else if (cycles < 4 && timer.seconds() < 22) {//execute only 3 cycles if enough time not left
